...
def homepage(request):
    ...
    if request.user.is_authenticated: 
        ...
        if current_sprint != None:
            ...
            # ax1.bar(day_of_week, hours_worked, linestyle='--', markersize=12, color='royalblue', marker='o', label="Hours worked")
            ax1.bar(day_of_week, hours_worked, label="Hours worked")
            ax1.set_xlabel("Day of Week")
            ax1.set_ylabel("Hours Worked", color='royalblue')
            ax2 = ax1.twinx()
            # ax2.bar(day_of_week, percentage_achieved, color='#ffffff', marker='o', label="Percentage Achieved")
            ax2.bar(day_of_week, percentage_achieved, label="Percentage Achieved")
            ax2.set_ylabel("Percentage Achieved", color='#463A14')
            ax1.set_facecolor("#36A2EB80") 
            plt.legend()
            plt.grid(False)
